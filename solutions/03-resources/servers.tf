...
resource "aws_instance" "game_server" {
  ...
  user_data = <<-EOF
    ...
    <body>
      ...
      <div class="status">
        ...
        <p><strong>Instance Type:</strong> ${var.game_server_instance_type}</p>
  ...
  # CloudWatch 에이전트를 위한 종속성 설정
  depends_on = [
    aws_iam_role_policy.game_server_cloudwatch_policy
  ]
}
